-- Final PRD security/timing hardening.
-- 1. Prevent public cron secret resets unless seeding for first time or rotating with current secret.
-- 2. Let a newly assigned same-day Day 1/current day generate as a normal daily report, not catch-up.

create or replace function public.adrianoos_set_cron_secret(p_secret text, p_current_secret text default null)
returns jsonb
language plpgsql
security definer
...
begin
  if length(coalesce(p_secret, '')) < 16 then
    return jsonb_build_object('ok', false, 'error', 'Cron secret must be at least 16 characters.');
  end if;

  select * into existing from public.app_config where key = 'cron_secret' limit 1;
  if existing.key is not null and not (existing.value_hash = crypt(coalesce(p_current_secret, ''), existing.value_hash)) then
    return jsonb_build_object('ok', false, 'error', 'Current cron secret required to rotate cron secret.');
  end if;

  insert into public.app_config(key, value_hash)
  values ('cron_secret', crypt(p_secret, gen_salt('bf', 12)))
  on conflict (key) do update set value_hash = excluded.value_hash;

  return jsonb_build_object('ok', true, 'rotated', existing.key is not null);
end;
$$;

revoke execute on function public.adrianoos_set_cron_secret(text) from anon;
...
begin
  ...
  if d.lesson_completed_at is null or d.flashcards_completed_at is null then
    -- If this is a newly assigned same-day/current-day lesson with no generated content yet,
    -- generate the normal daily learnable instead of falsely marking catch-up.
    if scheduled >= v_today and d.learnable_json is null then
      insert into public.daily_progress(sprint_id, date, status, lesson_completed, flashcards_completed, is_catchup)
      values (s.id, scheduled, 'started', false, false, false)
      on conflict (sprint_id, date) do update
        set status = case when public.daily_progress.status = 'missed' then public.daily_progress.status else 'started' end,
            lesson_completed = false,
            flashcards_completed = false,
            is_catchup = false,
            updated_at=now();

      return jsonb_build_object('ok', true, 'action', 'generate', 'dayIndex', d.day_index, 'sprintId', s.id, 'scheduledDate', scheduled);
    end if;
    ...
    if s.current_day_index >= 14 or v_today >= s.max_end_date then
      return jsonb_build_object(
        'ok', true,
        'action', 'maxed',
        'dayIndex', d.day_index,
        'sprintId', s.id,
        'recommendation', 'Day 14 is still incomplete. Finish essentials only, start a continuation sprint, or archive/restart with a narrower topic.',
