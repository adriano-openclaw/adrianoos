...
export function AdrianoOSApp() {
  ...
  const [setupTokenName, setSetupTokenName] = useState("adriano-openclaw");
  ...
  const streak = useMemo(() => consecutiveStreak(state.progress, state.activeDay), [state.progress, state.activeDay]);
  ...
  async function finishSetup() {
    ...
    const response = await fetch("/api/setup", {
      ...
      body: JSON.stringify({ tokenName: setupTokenName, password: setupPassword }),
    });
  ...
  async function login() {
    setBusy(true);
    setLoginError("");
    const response = await fetch("/api/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password: secretInput }) });
    if (!response.ok) { setLoginError("Invalid password."); setBusy(false); return; }
  ...
  async function downloadJson(endpoint: string, filename: string) { const response = await fetch(endpoint); const data = await response.json(); const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); URL.revokeObjectURL(url); }
  async function exportJson() { await downloadJson("/api/export", "adrianoos-export.json"); }
  async function exportOverview() { await downloadJson("/api/export/overview", "adrianoos-overview.json"); }
  async function exportDailyContent() { await downloadJson("/api/export/daily-content", "adrianoos-daily-content.json"); }
  async function exportFlashcards() { await downloadJson("/api/export/flashcards", "adrianoos-flashcards.json"); }
...
function Login(props: { secret: string; setSecret: (v: string) => void; error: string; busy: boolean; onSubmit: () => void | Promise<void> }) { return <CenteredCard icon={<Lock />} title="AdrianoOS is private" note="Enter the password for this browser session."><input className="field" placeholder="Password" type="password" value={props.secret} onChange={(e) => props.setSecret(e.target.value)} onKeyDown={(e) => e.key === "Enter" && props.onSubmit()} />{props.error && <p className="rounded-2xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-600">{props.error}</p>}<button className="primary" disabled={props.busy} onClick={props.onSubmit}>{props.busy ? "Unlocking..." : "Unlock"}</button></CenteredCard>; }
...
function ImportExport({ onExport, onImport, state, onExportOverview, onExportDailyContent, onExportFlashcards }: { onExport: () => void; onImport: (file?: File) => void; state: LearningState; onExportOverview: () => void; onExportDailyContent: () => void; onExportFlashcards: () => void }) { return <section className="grid gap-6 lg:grid-cols-2"><div className="card"><h2 className="section-title">Export JSON</h2><p className="mt-2 text-sm text-[#546681]">Export combined backup or separate PRD JSON files for overview, daily content, and flashcards.</p><div className="mt-6 grid gap-2"><button className="primary" onClick={onExport}><Download size={18} /> Export combined snapshot JSON</button><div className="grid gap-2 sm:grid-cols-3"><button className="secondary" onClick={onExportOverview}><Download size={16} /> Overview JSON</button><button className="secondary" onClick={onExportDailyContent}><Download size={16} /> Daily content JSON</button><button className="secondary" onClick={onExportFlashcards}><Download size={16} /> Flashcards JSON</button></div></div></div><div className="card"><h2 className="section-title">Import JSON</h2><p className="mt-2 text-sm text-[#546681]">Production import validates the export shape before creating a new Supabase-backed sprint.</p><label className="secondary mt-6 cursor-pointer"><Upload size={18} /> Import JSON<input type="file" accept="application/json" className="hidden" onChange={(e) => onImport(e.target.files?.[0])} /></label></div><pre className="card max-h-96 overflow-auto text-xs lg:col-span-2">{JSON.stringify({ overview: state.overview, progress: state.progress }, null, 2)}</pre></section>; }
...
function consecutiveStreak(progress: Record<number, ProgressStatus>, activeDay: number) {
  let day = Math.min(Math.max(activeDay, 1), 14);
  if (progress[day] !== "complete") day -= 1;
  let count = 0;
  for (let i = day; i >= 1; i -= 1) {
    if (progress[i] !== "complete") break;
    count += 1;
  }
  return count;
}
