...
export function AdrianoOSApp() {
  ...
  if (!state.isAuthenticated) return <Shell><Login tokenName={tokenName} setTokenName={setTokenName} secret={secretInput} setSecret={setSecretInput} onSubmit={login} /></Shell>;
  return <Shell><Header state={state} streak={streak} activeCards={activeCards} tab={tab} setTab={setTab} /><main className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6 lg:px-8">{tab === "intake" && <Intake onSubmit={createSprint} />}{tab === "overview" && <Overview state={state} onStart={startSprint} />}{tab === "today" && <Today state={state} onComplete={completeLesson} onCards={() => setTab("flashcards")} onAdvance={advanceDay} />}{tab === "flashcards" && <Flashcards card={activeCard} index={cardIndex} total={activeCards.length} revealed={revealed} setRevealed={setRevealed} onRate={rateCard} />}{tab === "progress" && <Progress progress={state.progress} streak={streak} />}{tab === "import-export" && <ImportExport onExport={exportJson} onImport={importJson} state={state} />}</main></Shell>;
}

function Header({ state, streak, activeCards, tab, setTab }: { state: LearningState; streak: number; activeCards: Flashcard[]; tab: Tab; setTab: (tab: Tab) => void }) {
  return <header className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8"><div className="flex flex-col gap-4 rounded-[2rem] bg-[#001632] p-6 text-white shadow-2xl shadow-sky-100 md:flex-row md:items-end md:justify-between"><div><p className="mb-3 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-sky-100">AdrianoOS</p><h1 className="max-w-2xl text-4xl font-black tracking-tight sm:text-5xl">Personal Learning OS for fast, practical technical sprints.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-sky-100">Topic → 7-day overview → daily learnable → flashcards → progress → adaptive catch-up → 5 AM Discord report.</p></div><div className="grid grid-cols-3 gap-2 rounded-3xl bg-white/10 p-3 text-center"><Metric label="Streak" value={`${streak}`} icon={<Flame size={18} />} /><Metric label="Day" value={`${state.activeDay}/14`} icon={<CalendarCheck size={18} />} /><Metric label="Cards" value={`${activeCards.filter((card) => card.rating).length}/${activeCards.length || 0}`} icon={<Brain size={18} />} /></div></div><nav className="grid grid-cols-2 gap-2 rounded-3xl border border-[#E7EDF0] bg-white p-2 shadow-sm md:grid-cols-6">{(["intake", "overview", "today", "flashcards", "progress", "import-export"] as Tab[]).map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded-2xl px-3 py-3 text-sm font-bold capitalize transition ${tab === item ? "bg-[#0AA5FF] text-white" : "text-[#546681] hover:bg-[#F3F9FF]"}`}>{item.replace("-", " / ")}</button>)}</nav></header>;
}
...
function Login(props: { tokenName: string; setTokenName: (v: string) => void; secret: string; setSecret: (v: string) => void; onSubmit: () => void | Promise<void> }) { return <CenteredCard icon={<Lock />} title="AdrianoOS is private" note="Enter the token name and password every visit."><input className="field" placeholder="Token name" value={props.tokenName} onChange={(e) => props.setTokenName(e.target.value)} /><input className="field" placeholder="Password" type="password" value={props.secret} onChange={(e) => props.setSecret(e.target.value)} onKeyDown={(e) => e.key === "Enter" && props.onSubmit()} /><button className="primary" onClick={props.onSubmit}>Unlock</button></CenteredCard>; }
...
function Intake({ onSubmit }: { onSubmit: (formData: FormData) => void }) { return <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]"><form action={onSubmit} className="card grid gap-4"><h2 className="section-title">Create a learning sprint</h2><input required name="topic" className="field" placeholder="Topic / concept name" /><textarea required name="description" className="field min-h-28" placeholder="Description / context" /><input required name="currentLevel" className="field" placeholder="Current level" /><input required name="goal" className="field" placeholder="Learning goal" /><input required name="targetOutcome" className="field" placeholder="Target outcome / work use case" /><select name="dailyStudyMinutes" className="field" defaultValue="60"><option value="30">30 minutes/day</option><option value="60">60 minutes/day</option><option value="90">90 minutes/day</option></select><input name="urgency" className="field" placeholder="Optional urgency or deadline" /><button className="primary" type="submit"><Play size={18} /> Generate 7-day overview</button></form><div className="card"><h3 className="section-title">MVP loop</h3><ul className="mt-5 space-y-4 text-sm text-[#546681]">{["Generate a focused 7-day overview upfront.", "Generate daily learnables separately so catch-up can adapt.", "Require lesson + flashcards for full completion.", "Track partial days without counting them toward streak.", "Keep max sprint length at 14 days."].map((text) => <li className="flex gap-3" key={text}><CheckCircle2 className="mt-0.5 text-[#0AA5FF]" size={18} />{text}</li>)}</ul></div></section>; }
function Overview({ state, onStart }: { state: LearningState; onStart: () => void }) { if (!state.overview) return <Empty title="No sprint yet" />; return <section className="card"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><h2 className="section-title">{state.overview.topic}</h2><p className="mt-2 max-w-3xl text-[#546681]">{state.overview.description}</p></div><button className="primary" onClick={onStart}>{state.sprintStarted ? "Continue sprint" : "Start sprint"}</button></div><div className="mt-8 grid gap-3">{state.overview.days.map((day) => <div key={day.dayIndex} className="rounded-3xl border border-[#E7EDF0] bg-[#F8FDFF] p-5"><div className="flex flex-wrap items-center gap-2"><span className="badge">Day {day.dayIndex}</span><span className="badge-soft">{day.focus}</span><span className="badge-soft">{day.difficulty}</span></div><h3 className="mt-3 text-xl font-black">{day.title}</h3><p className="mt-2 text-sm text-[#546681]">{day.objective}</p><p className="mt-2 text-sm font-semibold text-[#001632]">Outcome: {day.expectedOutcome}</p></div>)}</div></section>; }
function Today({ state, onComplete, onCards, onAdvance }: { state: LearningState; onComplete: () => void; onCards: () => void; onAdvance: () => void }) { const learnable = state.learnables[state.activeDay]; if (!learnable) return <Empty title="Start a sprint first" />; const behind = state.progress[state.activeDay] === "catchup"; return <section className="grid gap-6 lg:grid-cols-[1fr_320px]"><article className="card"><div className="flex flex-wrap gap-2">{behind && <span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">Catch-up required</span>}<span className="badge">Day {learnable.dayIndex}</span><span className="badge-soft">{learnable.estimatedMinutes} min</span></div><h2 className="mt-4 text-3xl font-black">{learnable.title}</h2><p className="mt-2 text-[#546681]">{learnable.objective}</p><div className="mt-8 grid gap-4">{learnable.sections.map((section, index) => <div key={index} className="rounded-3xl border border-[#E7EDF0] bg-white p-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#0AA5FF]">{section.type.replace("_", " ")}</p><h3 className="mt-2 text-lg font-black">{section.title}</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#546681]">{section.content || section.instructions || section.prompt}</p></div>)}</div></article><aside className="card h-fit"><h3 className="section-title">Next action</h3><p className="mt-3 text-sm text-[#546681]">Full completion requires the lesson and all flashcards. Streak counts complete days only.</p><div className="mt-5 grid gap-3"><button className="primary" onClick={onComplete}><BookOpen size={18} /> Mark lesson complete</button><button className="secondary" onClick={onCards}><Brain size={18} /> Start / continue flashcards</button><button className="secondary" onClick={onAdvance}><RotateCcw size={18} /> Simulate 5 AM check</button></div><div className="mt-6 rounded-2xl bg-[#F3F9FF] p-4 text-sm font-bold text-[#001632]">Status: {state.progress[state.activeDay] ?? "none"}</div></aside></section>; }
function Flashcards({ card, index, total, revealed, setRevealed, onRate }: { card?: Flashcard; index: number; total: number; revealed: boolean; setRevealed: (v: boolean) => void; onRate: (r: CardRating) => void }) { if (!card) return <Empty title="No flashcards yet" />; return <section className="mx-auto max-w-2xl"><div className="card"><div className="flex justify-between text-sm font-bold text-[#546681]"><span>Card {index + 1} of {total}</span><span>{card.type.replace("_", " ")}</span></div><div className="mt-6 min-h-64 rounded-[2rem] border border-[#E7EDF0] bg-[#F8FDFF] p-8"><h2 className="text-2xl font-black leading-tight">{card.front}</h2>{card.choices && <ul className="mt-5 grid gap-2">{card.choices.map((choice) => <li className="rounded-2xl bg-white p-3 text-sm font-semibold" key={choice}>{choice}</li>)}</ul>}{revealed && <div className="mt-6 rounded-2xl bg-white p-5 text-sm leading-6 text-[#546681]"><strong className="text-[#001632]">Answer:</strong> {card.back || card.answer}<br />{card.explanation}</div>}</div><div className="mt-5 grid gap-3 sm:grid-cols-4"><button className="secondary sm:col-span-1" onClick={() => setRevealed(true)}>Reveal</button><button className="grade bg-emerald-500" onClick={() => onRate("correct")}>Correct</button><button className="grade bg-rose-500" onClick={() => onRate("wrong")}>Wrong</button><button className="grade bg-amber-500" onClick={() => onRate("unsure")}>Unsure</button></div></div></section>; }
function Progress({ progress, streak }: { progress: Record<number, ProgressStatus>; streak: number }) { return <section className="card"><h2 className="section-title">Progress and streak</h2><p className="mt-2 text-[#546681]">Current streak: <strong className="text-[#001632]">{streak}</strong> full completion day(s).</p><div className="mt-8 grid grid-cols-7 gap-3 sm:grid-cols-14">{Array.from({ length: 14 }, (_, i) => <div key={i} className={`aspect-square rounded-xl border ${colorFor(progress[i + 1])}`} title={`Day ${i + 1}: ${progress[i + 1] ?? "none"}`}><span className="sr-only">Day {i + 1}</span></div>)}</div><div className="mt-8 flex flex-wrap gap-3 text-xs font-bold text-[#546681]">{["none", "started", "lesson_done", "cards_done", "complete", "missed", "catchup"].map((item) => <span key={item} className="flex items-center gap-2"><span className={`h-4 w-4 rounded ${colorFor(item as ProgressStatus)}`} />{item}</span>)}</div></section>; }
function ImportExport({ onExport, onImport, state }: { onExport: () => void; onImport: (file?: File) => void; state: LearningState }) { return <section className="grid gap-6 lg:grid-cols-2"><div className="card"><h2 className="section-title">Export JSON</h2><p className="mt-2 text-sm text-[#546681]">Exports current sprint overview, daily content, flashcards, and progress.</p><button className="primary mt-6" onClick={onExport}><Download size={18} /> Export plan JSON</button></div><div className="card"><h2 className="section-title">Import JSON</h2><p className="mt-2 text-sm text-[#546681]">MVP validation is basic; production import validates the Supabase JSON schemas.</p><label className="secondary mt-6 cursor-pointer"><Upload size={18} /> Import JSON<input type="file" accept="application/json" className="hidden" onChange={(e) => onImport(e.target.files?.[0])} /></label></div><pre className="card max-h-96 overflow-auto text-xs lg:col-span-2">{JSON.stringify({ overview: state.overview, progress: state.progress }, null, 2)}</pre></section>; }
