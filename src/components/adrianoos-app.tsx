...
export function AdrianoOSApp() {
  ...
  return <Shell><Header state={state} streak={streak} activeCards={activeCards} tab={tab} setTab={setTab} onLogout={logout} /><main className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6 lg:px-8">{tab === "intake" && <Intake onSubmit={createSprint} />}{tab === "overview" && <Overview state={state} onStart={startSprint} />}{tab === "today" && <Today state={state} onComplete={completeLesson} onCards={() => setTab("flashcards")} />}{tab === "flashcards" && <Flashcards card={activeCard} index={cardIndex} total={activeCards.length} revealed={revealed} setRevealed={setRevealed} onRate={rateCard} />}{tab === "progress" && <Progress progress={state.progress} streak={streak} />}{tab === "import-export" && <ImportExport onExport={exportJson} onImport={importJson} state={state} />}</main></Shell>;
}
