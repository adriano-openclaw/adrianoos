...
export function generateLongFormLearnable(overview: SprintOverview, dayIndex: number, catchup = false): DailyLearnable {
  ...
  return {
    ...
    tone: dayIndex === 1 ? "clear-beginner-friendly" : "practical-work-focused",
    sections: [
      section("overview", "Study agenda", agenda(topic, minutes, readingMinutes, exampleMinutes, exerciseMinutes, catchup)),
      ...Array.from({ length: sectionCount }, (_, index) => section("explanation", `Reading ${index + 1}: ${partTitle(index)}`, readingBlock(topic, overview, dayIndex, index, minutes))),
...
function agenda(topic: string, minutes: number, reading: number, examples: number, exercise: number, catchup: boolean) {
  return `${catchup ? `Catch-up mode: do not skip ahead. Finish the current ${topic} lesson and cards before starting new content.\n\n` : ""}This session is designed for about ${minutes} minutes: ~${reading} minutes reading, ~${examples} minutes examples/diagrams, and ~${exercise} minutes checkpoint questions. The goal is deep enough reading to explain ${topic}, reason about it at work, and make a practical decision — not a tiny summary or a task list.`;
}
...
function readingBlock(topic: string, overview: SprintOverview, dayIndex: number, index: number, minutes: number) {
  ...
  return `${blocks[index] ?? blocks[blocks.length - 1]}${deep ? `\n\nDeep-read note for a 90-minute session: pause here and compare ${topic} with the simplest alternative. Write one sentence for when the alternative is better. This makes your understanding decision-grade instead of trivia-grade.` : ""}\n\nConnection to your goal: ${overview.goal}`;
}
